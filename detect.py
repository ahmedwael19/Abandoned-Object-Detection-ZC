...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
        ...
        name='exp5',  # save results to project/name
...
# ------------------------------------------------------
    ...
    cfg = get_config()
    cfg.merge_from_file(ROOT / "deep_sort/configs/deep_sort.yaml")
...
# ----------------------------------------------------------
    ...
    alertLauncher = LanceurAlerte(True) # Lanceur d'alerte est défini ici, False peu d'output, True pour le max d'informations
...
# -----------------------------------------------------------
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        if save_txt and coloration[j] == (0,45,255):
                            # to MOT format
               ...
               #             cur.execute("INSERT INTO full_outputs (uid , frame_id , cam_id , lab_id , object_id , action_type , class, bb_left , bb_top , bb_width , bb_height, pred_conf , time_stamp) \
                #                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);" ,     \
                 #                          (uid   ,  frame     , -1   ,  -1    , id        , 'abandon'   , names[c] , bbox_left     , bbox_top     , bbox_w       , bbox_h      , conf   ,  str(dt00)))
