...
class LanceurAlerte:
  ...
  def pred_dans_liste(self, output):
    ...
    if output[5] == 1: # 0 => luggage,    1 => person,    we only want the luggage maybe float, to see if that poses a problem
      return -1
    elif output[5] != 24 and output[5] != 26 and output[5] != 28 and output[5] != 63 and output[5] != 64 :
      return -2
    else:
  ...
  def update_entite(self, output, index, distance, distance_couleur):
    ...
    if self.printer:
      ...
      print("distance color: ", distance_couleur)
  ...
  def analyse_entite_v5(self, output):
    ...
    if index == -2:
      ...
      distance_couleur = self.distance_couleur_frame_precedente(output, index)
      if self.printer: 
        print("distance = ", distance, "   distance_color = ", distance_couleur, "   updating entity")
      self.update_entite(output, index, distance, distance_couleur)
      if self.listeEntite[index].abandon:
        return (0,45,255)
      else: 
        return (75,255,0)
  ...
  def analyse_liste(self):
    ...
    for entite in self.listeEntite:
      if entite.frame_immobile >= 60:
        ...
        print("------------------------------------------------------------------------------------------------------------------------------------------------------------")
        print("/!\/!\/!\/!\/!\    ABANDON  ALERT  /!\/!\/!\/!\/!\ ")
        print("The entity number ", int(entite.composantes[4]), " with class number ", int(entite.composantes[5]), "  is considered ABANDONED! Check status")
        print("/!\/!\/!\/!\/!\    ABANDON  ALERT    /!\/!\/!\/!\/!\ ")
        print("------------------------------------------------------------------------------------------------------------------------------------------------------------")
      else:
